...
def plot_beams_polar(
    ...
    """
    Create a polar plot of beam pointings.
    ...
    beam_radius_deg : float
        Approximate beam radius for visualization (default: 4 deg).
    figsize : tuple
    ...
    for i, p in enumerate(pointings):
        ...
        # Plot beam center
        ax.scatter(az_rad, zenith_angle, s=200, c=[colors[i % 10]],
                   edgecolors='black', linewidths=1.5, zorder=5)

        # Add beam circle (approximate beam size)
        circle = Circle(
            (az_rad, zenith_angle), beam_radius_deg,
            transform=ax.transData._b,
            fill=False, edgecolor=colors[i % 10], linewidth=2, alpha=0.5
        )

        # Label
        label = p.name if p.name else f"Beam {i}"
        # Offset label slightly
        label_offset = beam_radius_deg + 2
        ax.annotate(
            label,
            (az_rad, zenith_angle + label_offset),
            fontsize=8,
            ha='center',
            va='bottom',
            color=colors[i % 10],
    ...
    # Add legend with beam info
    legend_text = []
    for i, p in enumerate(pointings):
        name = p.name if p.name else f"Beam {i}"
        legend_text.append(f"{name}: Alt={p.alt_deg:.1f}, Az={p.az_deg:.1f}")

    # Add text box with beam info
    textstr = '\n'.join(legend_text)
    props = dict(boxstyle='round', facecolor='wheat', alpha=0.8)
    fig.text(0.02, 0.02, textstr, fontsize=8, verticalalignment='bottom',
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="Approximate beam radius in degrees (default: 4.0)",
    )
    parser.add_argument(
        "--no-show",
        action="store_true",
    ...
    if args.style in ["polar", "both"]:
        fig1, ax1 = plot_beams_polar(
            ...
            beam_radius_deg=args.beam_radius,
        )
